...
export function render(containerEl: HTMLElement, plugin: GlimpsePlugin, tab: SettingTab) {
  ...
  allGroups.forEach(g => {
    ...
    if (g === tab.activeGroup) tabEl.addClass("active");
    ...
    tabEl.addEventListener("drop", e => {
      e.preventDefault();
      tabEl.removeClass("drag-hover");
      if (!tab._dragItemId || g === tab.activeGroup) return;
      const draggedId = tab._dragItemId;
      tab._dragItemId = undefined;
  ...
  renameGroupBtn.setIcon("folder-pen").setClass("clickable-icon").setTooltip("重命名分组").onClick(() => {
    ...
    const modal = new Modal(plugin.app);
    modal.titleEl.setText(`重命名分组"${tab.activeGroup}"`);
    const input = new TextComponent(modal.contentEl);
    input.inputEl.addClass("glimpse-modal-input");
    input.setValue(tab.activeGroup);
    input.inputEl.focus();
    ...
    new Setting(modal.contentEl)
      ...
      .addButton(b => b.setButtonText("确认").setClass("mod-cta").onClick(() => {
        ...
        if (!newName || newName === tab.activeGroup) { modal.close(); return; }
        ...
        config.queryOrder.forEach(h => {
          if (config.queries[h]?.group === tab.activeGroup) config.queries[h].group = newName;
        });
        const idx = config.groups?.indexOf(tab.activeGroup);
        if (idx !== undefined && idx > -1) config.groups![idx] = newName;
        tab.activeGroup = newName;
        plugin.saveSettings();
        tab.display();
        modal.close();
      }));
  ...
  delGroupBtn.setIcon("delete").setClass("clickable-icon").onClick(async () => {
    ...
    if (groupHighlighters.length > 0) {
      const confirmed = await new Promise<boolean>(resolve => {
        ...
        confirmModal.contentEl.createEl("p", { text: `确定删除分组"${tab.activeGroup}"？该组包含 ${groupHighlighters.length} 个高亮器。` });
  ...
  const filteredOrder = config.queryOrder.filter(h => {
    const q = config.queries[h];
    if (tab.activeGroup === "默认") return !q.group || q.group === "默认";
    return q.group === tab.activeGroup;
  });
      ...
      .addButton(button => {
        button
          ...
          .onClick(() => {
            ...
            new ExportModal(plugin.app, plugin, highlighter, data).open();
          });
      })
      .addButton(button => {
        button
  ...
  const sortableEl = Sortable.create(highlightersContainer, {
    ...
    onSort: async command => {
      ...
      plugin.settings.staticHighlighter.queryOrder = config.queryOrder;
      await plugin.saveSettings();
    },
  });
}
