 ...
 * Node 内置 crypto（Obsidian `window.require('crypto')`）负责 AES；RSA 模幂用 BigInt。
 */
import { md5hex } from "./kugouMusic";

/** 获取 Node crypto 模块：Obsidian 渲染进程走 window.require（绕过 CSP） */
...
/**
 * weapi 加密：返回 `{params, encSecKey}` 供 POST 表单。
...
/**
 * eapi 加密：`md5("nobody"+path+"use"+payload+"md5forencrypt")`
...
/** 网易云账号响应解析：ok 仅表示登录有效（code===200），vipType 单独返回（0=普通账号，非 0=会员） */
...
/** 账号登录态细分解析：code===200 不代表已登录（匿名 Cookie 同样返回 200 但 profile=null），
 *  必须以 profile.userId 判定登录；uid 供账号歌单接口使用 */
export function parseAccountLoginState(raw: string): {
  ok: boolean; loggedIn: boolean; uid: number; nickname?: string; vipType: number;
} {
  try {
    const data = JSON.parse(raw);
    const code = Number(data?.code ?? -1);
    const uid = Number(data?.profile?.userId ?? data?.account?.id ?? 0);
    return {
      ok: code === 200,
      loggedIn: code === 200 && uid > 0,
      uid,
      nickname: data?.profile?.nickname || undefined,
      vipType: Number(data?.profile?.vipType ?? 0),
    };
  } catch {
    return { ok: false, loggedIn: false, uid: 0, vipType: 0 };
  }
}
...
/** 解析批量歌曲详情响应：songs[] 含 id/name/ar[].name/al.name+picUrl/dt + h/m/l 音质档（size/br） */
export function parseSongDetailSongs(raw: string): Array<{
  id: number; name: string; artist: string; album?: string; coverUrl?: string; duration?: number; fee?: number; size?: number; bitrate?: number;
}> {
  try {
    const data = JSON.parse(raw);
