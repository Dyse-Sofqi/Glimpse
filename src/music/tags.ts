/**
 * 音频标签读写（MP3 走 node-id3，M4A 走自研 MP4 atom 写入器，FLAC/OGG 只读解析）。
 * 支持 vault 内文件与库外 Windows 盘符绝对路径（fs 直读直写）。
 */
import * as NodeID3 from "node-id3";
import * as jsmediatags from "jsmediatags";
import { type App, TFile } from "obsidian";
import { extractFlacLyrics, extractFlacTags } from "./bytes/vorbis";
import { extractOggTags } from "./bytes/ogg";
import { estimateMp3Duration } from "./mp3Duration";
import { writeMp4Tags, extractMp4Text, extractMp4Cover, extractMp4Duration, type Mp4TagSet } from "./bytes/mp4";
import { detectAudioContainer } from "./songScanner";
import { blobOf, asArrayBuffer } from "./shared";
...
export interface AudioSource {
  type: "vault" | "external" | "blob";
  file?: TFile;
  path?: string;
  /** blob 来源（在线试听）：可播放 URL，生命周期移交播放器（销毁时 revoke） */
  url?: string;
}

/** 可编辑音频路径判别：MP3 / M4A（M4A 标签写入支持；编辑弹窗共用） */
function isEditableAudioPath(p: string): boolean {
  return /\.(mp3|m4a)$/i.test(p);
}

/** 判断外部路径是否可编辑（暴露给弹窗用） */
export function isEditableSource(source: AudioSource): boolean {
  const name = source.type === "vault" ? source.file?.name ?? "" : source.path ?? "";
  return isEditableAudioPath(name);
}
...
async function writeFileBuffer(app: App, source: AudioSource, data: Uint8Array): Promise<boolean> {
  try {
    if (source.type === "vault" && source.file) {
      await app.vault.modifyBinary(source.file, asArrayBuffer(data));
      return true;
    }
    if (source.type === "external" && source.path) {
      const fs = (window as any).require("fs");
...
/** 桌面环境：读取音频文件头部 N 字节（时长估算用，不读全文件）。失败返回 null */
export async function readAudioHeadBytes(app: App, source: AudioSource, length = 65536): Promise<Uint8Array | null> {
  try {
    const fs = (window as any).require("fs");
    let fullPath: string | null = null;
    if (source.type === "external" && source.path) {
 ...
 *  moov 可能在文件头部（faststart）或尾部：先读头/尾 64KB 窗口定位 moov 起点与大小，
      ...
      const findMoov = (buf: Buffer, base: number): { start: number; size: number } | null => {
        ...
        while (offset + 8 <= buf.length) {
          ...
          if (size === 1) {
            ...
            const hi = ((buf[offset + 8] << 24) | (buf[offset + 9] << 16) | (buf[offset + 10] << 8) | buf[offset + 11]) >>> 0;
            const lo = ((buf[offset + 12] << 24) | (buf[offset + 13] << 16) | (buf[offset + 14] << 8) | buf[offset + 15]) >>> 0;
...
function buildTagPatch(tags: Mp3Tags): Record<string, any> {
  ...
  if (tags.cover !== undefined) {
    patch.image = tags.cover
      ...
      : undefined; // 移除封面：node-id3 update({image:undefined}) 会丢弃 APIC 帧
  }
  return patch;
}
...
export function embedTagsIntoBytes(bytes: Uint8Array, tags: Mp3Tags): Uint8Array | null {
  try {
    ...
    return updated instanceof Uint8Array ? new Uint8Array(updated) : null;
  } catch {
    return null;
  }
}
...
/**
 * 写标签（MP3）：node-id3.update → 写回 → 回读校验（长度与可解析性）→ 失败还原。
 * 移除封面：cover 传 null；歌词/注释传空串表示移除。
 */
async function writeMp3Tags(app: App, source: AudioSource, tags: Mp3Tags): Promise<boolean> {
  const buf = await readFileBuffer(app, source);
  ...
  try {
    ...
    if (!(await writeFileBuffer(app, source, updatedBytes))) {
      await writeFileBuffer(app, source, original); // 还原
      return false;
    }
    const verify = await readFileBuffer(app, source);
    // 精确比对回读长度与写入长度；只有部分写入/截断等异常才会长度不一致，配合可解析性兜底
    const lengthOk = !!verify && verify.byteLength === updatedBytes.byteLength && verify.byteLength > 512;
    const readable = verify ? parseTags(verify) !== null : false;
    // 音频完整性校验：标签区后应能遍历出 MPEG 帧（估计时长 >0）
 ...
 * MP3/unknown 走 writeMp3Tags；FLAC/OGG 无写入器直接拒绝（防写坏文件）。
 */
...
/** 从 M4A moov 字节解析标签（歌单富化用）：文本原子走自研解析器，封面自研读取 */
...
/** 用 jsmediatags 读取跨格式标签（FLAC/M4A/MP3 等）+ 自研 OGG/FLAC 歌词解析，只读，供非 MP3「查看标签」 */
export async function readGenericTags(bytes: Uint8Array): Promise<Mp3Tags | null> {
  ...
  // FLAC：Vorbis comment 直接读全字段（jsmediatags 的 FLAC 读取器丢弃 LYRICS）
  if (isFlac) {
    const map = extractFlacTags(bytes);
    if (map && map.size > 0) {
      const out: Mp3Tags = {};
      const get = (k: string) => map.get(k);
      if (get("TITLE")) out.title = get("TITLE");
      if (get("ARTIST")) out.artist = get("ARTIST");
      if (get("ALBUM")) out.album = get("ALBUM");
      if (get("DATE") || get("YEAR")) out.year = get("DATE") || get("YEAR");
    ...
    try {
      ...
      jsmediatags.read(Buffer.from(bytes), {
        onSuccess: (result) => {
          ...
          // M4A：jsmediatags 读自研 8 字节 data box 头文本原子失败时用自研解析器兜底补齐
          if (isM4a) {
            if (typeof out.title !== "string" || !out.title) { const v = extractMp4Text(bytes, "\u00a9nam"); if (v) out.title = v; }
            if (typeof out.artist !== "string" || !out.artist) { const v = extractMp4Text(bytes, "\u00a9ART"); if (v) out.artist = v; }
            if (typeof out.album !== "string" || !out.album) { const v = extractMp4Text(bytes, "\u00a9alb"); if (v) out.album = v; }
            if (typeof out.lyrics !== "string" || !out.lyrics) { const v = extractMp4Text(bytes, "\u00a9lyr"); if (v) out.lyrics = v; }
            if (!out.cover) { const v = extractMp4Cover(bytes); if (v) out.cover = v; }
          }
          resolve(out);
        },
        onError: () => {
          // jsmediatags 整体失败：M4A 文本字段走自研解析器（封面同样自读）
          if (isM4a) {
            const out: Mp3Tags = {};
            const title = extractMp4Text(bytes, "\u00a9nam");
            const artist = extractMp4Text(bytes, "\u00a9ART");
            ...
            if (lyrics) out.lyrics = lyrics;
            if (cover) out.cover = cover;
            resolve(title || artist || album || lyrics || cover ? out : null);
          } else {
            resolve(null);
          }
        },
...
/** 跨格式解析音频时长（秒）：MP3 逐帧估算 / M4A mvhd / 其他 null */
export function estimateAudioDuration(bytes: Uint8Array): number | null {
  const container = detectAudioContainer(bytes);
  if (container === "m4a") return extractMp4Duration(bytes);
  if (container === "mp3" || container === "unknown") return estimateMp3Duration(bytes);
  return null;
}
 ...
 */
export async function resolvePlayableUrl(app: App, source: AudioSource): Promise<string> {
  if (source.type === "vault" && source.file) {
  ...
  if (source.type === "external" && source.path) {
    try {
      ...
      return URL.createObjectURL(blobOf(new Uint8Array(buf), mime[ext] || "audio/mpeg"));
    } catch {
      // 文件被移动/删除/权限不足：返回 ''，编辑器对空 URL 优雅降级
      return "";
    }
  }
  return "";
...
/** 构造音频的 AudioSource：Windows 盘符绝对路径 → external（fs 直读）；否则按 vault 内路径找 TFile */
export function resolveAudioSourceByPath(app: App, path: string): AudioSource | null {
  const abs = /^[A-Za-z]:[\\/]/.test(path);
  if (abs) return { type: "external", path };
