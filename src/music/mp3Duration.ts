/**
 * MP3 时长解析：从 MP3 字节流估算时长。
 * 优先解析首帧头 + Xing/Info 头帧数（VBR 精确），否则逐帧遍历累加。
 */
...
/**
 * 估算 MP3 时长（仅凭文件头 + 文件总大小，不读全文件）——列表批量展示用。
 *  1. 跳过 ID3v2 找首个合法帧头；
 *  2. Xing/Info 含帧数 → 帧数 × 每帧采样数 ÷ 采样率（VBR 精确）；
 *  3. 否则按 CBR 推算：(文件大小 − 音频起始偏移) × 8 ÷ 码率（CBR 精确，VBR 无 Xing 为近似）。
 * head 需覆盖 ID3v2 标签区与首个帧头（建议 ≥ 64KB）；解析失败返回 null。
 */
export function estimateMp3DurationFromHead(head: Uint8Array, fileSize: number): number | null {
  const start = id3v2Size(head);
  let headerOffset = -1;
  let frame: MpegFrame | null = null;
  for (let i = start; i + 4 <= head.length; i++) {
    const f = parseFrameHeader(head, i);
    if (f) { headerOffset = i; frame = f; break; }
  }
  if (!frame || headerOffset < 0 || !frame.sampleRate || !frame.bitrateKbps) return null;

  // ① Xing/Info 帧数（VBR 精确，O(1)）
  for (let i = headerOffset + 4; i + 12 <= head.length && i <= headerOffset + 36; i++) {
    const isXing = head[i] === 0x58 && head[i + 1] === 0x69 && head[i + 2] === 0x6e && head[i + 3] === 0x67;
    const isInfo = head[i] === 0x49 && head[i + 1] === 0x6e && head[i + 2] === 0x66 && head[i + 3] === 0x6f;
    if (isXing || isInfo) {
      const flags = readUint32BE(head, i + 4);
      if (flags & 0x1) {
        const frames = readUint32BE(head, i + 8);
  ...
  // ② CBR 推算
  const audioBytes = fileSize - headerOffset;
  if (audioBytes <= 0) return null;
  const sec = (audioBytes * 8) / (frame.bitrateKbps * 1000);
 ...
 *  3. 否则逐帧遍历累加每帧采样数（无论 CBR/VBR、有无 Xing 都精确），
