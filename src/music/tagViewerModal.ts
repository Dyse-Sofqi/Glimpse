 ...
 * 仅展示标题/艺术家/专辑/歌词/封面，不提供编辑；入口在歌单弹窗的「查看标签」按钮。
 */
...
export default class TagViewerModal extends Modal {
  ...
  async onOpen() {
    ...
    this.contentEl.createDiv({ cls: "gm-tag-loading", text: "正在读取标签…" });
  ...
  private render(tags: Mp3Tags | null) {
    ...
    if (t.cover) {
      const url = URL.createObjectURL(blobOf(t.cover.data, t.cover.mime));
      const img = this.contentEl.createEl("img", { cls: "gm-tag-cover-img" });
      img.src = url;
      const revoke = () => URL.revokeObjectURL(url);
      img.onload = revoke;
      img.onerror = revoke;
    } else {
      this.contentEl.createDiv({ cls: "gm-tag-cover-preview", text: "（无封面）" });
    }

    const actions = this.contentEl.createDiv({ cls: "gm-tag-actions" });
    actions.createEl("button", { text: "关闭", cls: "mod-cta gm-tag-cancel" })
