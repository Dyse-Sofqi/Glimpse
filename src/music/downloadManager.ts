 ...
 * 流程：多源搜索（合并结果）→ 按来源分发下载 → 内嵌标签（MP3/M4A，含歌词/封面）→ 写 vault → 重扫。
 */
...
import {
  encryptWeApi, encryptEApi, parseVipAccountResponse, parseAccountLoginState,
...
const QQ_PROBE_SONGMID = "002wMg3O3K7UGG";
...
const NETEASE_USER_PLAYLIST_API = "https://music.163.com/weapi/user/playlist";
...
export interface DownloadSong {
  ...
  /** qq 专用：songmid */
  songmid?: string;
  /** VIP 受限标记：网易云 fee>0 / QQ pay.dowload=1 / 酷狗 privilege=10，仅标注不屏蔽 */
  vip?: boolean;
...
/** 推荐歌单来源平台 */
export type PlaylistSource = MusicSource;
...
export async function searchCandidates(
  ...
  // 各平台独立「请求+解析+映射」为 DownloadSong[]（VIP 歌曲不屏蔽仅标注）
  ...
  const mapQq = (text: string): DownloadSong[] =>
    parseQqSearchResponse(text).map((s) => ({
      ...
      album: s.album, duration: s.duration, size: s.size, bitrate: s.bitrate,
  ...
  if (out.length === 0 && onEmpty) {
    const enabledCount = MUSIC_SOURCES.filter((k) => isOn(k)).length;
    if (enabledCount > 0) {
  ...
  // 排序：纯关键词相关度（标题/艺术家命中关键词的分高排前）→ 标题短 → 字典序，不按平台来源排序
  return out.sort((a, b) => {
...
export async function fetchPlaylistSongs(source: PlaylistSource, playlistId: string): Promise<DownloadSong[]> {  switch (source) {
...
/**
 * 网易云账号歌单（需 Cookie）：account/get 拿 uid → user/playlist 分页拉全部（创建 + 收藏）。
 * 返回与推荐歌单同构的 RecommendedPlaylist（source=netease），复用现有歌单卡片/歌内曲目视图。
...
export async function fetchNeteaseAccountPlaylists(cookie: string): Promise<RecommendedPlaylist[]> {
  ...
  if (!info.loggedIn) {
    throw new Error("Cookie 未包含登录凭证（缺 MUSIC_U）：新版网页登录的 MUSIC_U 是 HttpOnly，document.cookie 读不到。请在 music.163.com 登录后按 F12 → 网络 面板刷新，复制任一请求请求头里的完整 Cookie（需含 MUSIC_U=…）再粘贴");
  }
  const uid = info.uid;
  // 2. user/playlist 分页（每页 200，上限 10 页防失控；创建 + 收藏都在返回里）
  ...
  for (let offset = 0; offset < 2000; offset += 200) {
    ...
    try {
      ...
      lists = Array.isArray(data?.playlist) ? data.playlist : [];
      more = data?.more === true;
    } catch { /* 解析失败按空页处理 */ }
    for (const p of lists) {
      ...
      out.push({
        ...
        creator: p?.creator && typeof p.creator === "object" && "nickname" in p.creator
          ? String((p.creator as { nickname?: unknown }).nickname ?? "")
          : undefined,
      });
    }
    if (!more || lists.length === 0) break;
  }
  return out;
}

/** 本地是否已有该歌（按下载命名规则检查 mp3/flac/m4a 三种扩展名变体），账号歌单同步跳过已存在用 */
export async function localSongExists(plugin: MusicManager, song: DownloadSong): Promise<boolean> {
  for (const ext of ["mp3", "flac", "m4a"]) {
    const target = await resolveDownloadTarget(plugin, buildSongFilename(song.artist, song.name, ext));
    if (await fileExists(plugin, target)) return true;
  }
  return false;
}
 ...
 * netease：weapi/nuser/account/get（code===200 且 vipType≠0 为有效会员）；
...
export async function testPlatformConnection(
  ...
  if (source === "netease") {
    ...
    const info = parseAccountLoginState(new TextDecoder().decode(res.data));
    if (!info.ok) return { ok: false, message: "网易云 Cookie 无效或已过期，请重新登录复制" };
    if (!info.loggedIn) {
      return {
        ok: false,
        message: "Cookie 未包含登录凭证（缺 MUSIC_U）：新版网页登录的 MUSIC_U 是 HttpOnly，document.cookie 读不到。请在 music.163.com 登录后按 F12 → 网络 面板刷新，复制任一请求请求头里的完整 Cookie（需含 MUSIC_U=…）再粘贴",
      };
    }
...
/** 按来源分发下载（onProgress 上报阶段/百分比）；一次失败自动重试（网络抖动友好） */
...
/** 试听音频会话内缓存：key=`${source}:${id}`，命中直接播放不重新拉取；模块级存活至 Obsidian 重启 */
...
/** 一键清除试听缓存（设置页「释放缓存」按钮调用） */
export function clearPreviewCache(): void {
  previewCache.clear();
}

/**
 * 试听歌曲：按来源拉取标准档音频字节（**不写盘**），供下载弹窗「试听」按钮转 Blob 播放。
...
export async function previewAudio(
  ...
  try {
    ...
    if (song.source === "netease" && song.neteaseId) {
      ...
      // 客户端缓存目录命中 → 离线秒开（不联网）
      const local = await findNeteaseCacheBytes(neteaseCacheFolder, song.neteaseId);
      if (local) {
        previewCache.set(cacheKey, { data: local.data, ext: local.ext });
        return { ok: true, message: "已从客户端缓存播放", data: local.data, ext: local.ext, fromCache: true };
      }
    ...
    if (song.source === "qq") {
      ...
      if (!purl) return { ok: false, message: "获取 QQ 播放地址失败（可能付费受限：VIP 歌曲需绿钻账号的有效 Cookie）" };
      const audioUrl = /^https?:\/\//i.test(purl) ? purl : `https://dl.stream.qqmusic.qq.com/${purl}`;
      const audioRes = await httpGet(audioUrl, 0, bytesProgress(`正在试听 ${song.name}`), qqHeaders);
      const audio = audioRes?.data;
      if (!audio || audio.byteLength < MIN_AUDIO_BYTES) return { ok: false, message: "试听失败（可能付费受限：VIP 歌曲需绿钻账号的有效 Cookie）" };
      previewCache.set(cacheKey, { data: audio, ext: toExt(audio) });
      return { ok: true, message: "", data: audio, ext: toExt(audio) };
    }
...
/**
 * 网易云客户端缓存目录命中：客户端把播放过的歌存为 `<歌曲ID>-<码率>.mp3/.flac`，
 * 按前缀查找并取最大文件（通常为最高码率）。未配置目录/未命中返回 null。
 */
async function findNeteaseCacheBytes(
  cacheFolder: string,
  neteaseId: number,
): Promise<{ data: Uint8Array; ext: "mp3" | "flac" } | null> {
  const folder = (cacheFolder || "").trim();
  if (!folder || !neteaseId) return null;
  try {
    const req = (window as any).require;
    const fs = req("fs");
    const pathMod = req("path");
    const entries: string[] = await fs.promises.readdir(folder);
    const prefix = `${neteaseId}-`;
    let best: string | null = null;
    let bestSize = 0;
    for (const name of entries) {
      if (!name.startsWith(prefix) || !/\.(mp3|flac)$/i.test(name)) continue;
      const full = pathMod.join(folder, name);
      const st = await fs.promises.stat(full).catch(() => null);
      if (st && st.isFile() && st.size > bestSize) {
        bestSize = st.size;
        best = full;
      }
    }
    if (!best || bestSize < MIN_AUDIO_BYTES) return null;
    const buf = await fs.promises.readFile(best);
    const data = new Uint8Array(buf);
    return { data, ext: looksLikeFlac(data) ? "flac" : "mp3" };
  } catch {
    return null;
  }
}
...
/**
 * 下载网易云歌曲到音频文件夹（歌词+封面内嵌）；onProgress 上报阶段/百分比。
...
export async function downloadNetEase(
  ...
  // 客户端缓存目录命中 → 直接使用（离线可用，音质为客户端当时缓存的档位）
  const local = await findNeteaseCacheBytes(plugin.getSettings().neteaseCacheFolder, song.id);
  if (local) {
    audio = local.data;
    qualityNote = "（客户端缓存）";
  }
...
// --- 网易云云盘（列表，需 MUSIC_U Cookie） ---

const NETEASE_CLOUD_GET_API = "https://music.163.com/weapi/v1/cloud/get";

/** weapi 加密参数转 POST 表单 */
function weapiForm(p: { params: string; encSecKey: string }): string {
  return `params=${encodeURIComponent(p.params)}&encSecKey=${encodeURIComponent(p.encSecKey)}`;
}

/** 云盘已存歌曲索引：ID（匹配曲库的上传 ID 与歌单曲目一致）+ 文件名（未匹配上传得到新 ID 按文件名兜底）
 *  + 歌名|歌手（网易云按音频内容判重：同一音频可能以别的文件名/新 ID 存在，靠名称兜底识别） */
export interface NeteaseCloudIndex {
  ids: Set<string>;
...
export async function fetchNeteaseCloudIndex(cookie: string): Promise<NeteaseCloudIndex> {
  ...
  for (let offset = 0, page = 0; page < 40; offset += limit, page++) {
    ...
    try {
      ...
      if (Number(json?.code ?? 0) !== 200) break;
      data = Array.isArray(json?.data) ? json.data : [];
    } catch { break; }
    for (const item of data) {
      const id = String((item as any)?.simpleSong?.id ?? (item as any)?.songId ?? "");
      if (id) ids.add(id);
      const fn = String((item as any)?.fileName ?? "").trim().toLowerCase();
      if (fn) fileNames.add(fn);
      const nm = String((item as any)?.songName ?? "").trim().toLowerCase();
      const ar = String((item as any)?.artist ?? "").trim().toLowerCase();
      if (nm) titles.add(`${nm}|${ar}`);
    }
    if (data.length < limit) break;
  }
  return { ids, fileNames, titles };
}

/**
 * VIP 下载链路：先 weapi 校验会员，再尝试 eapi（无损）→ weapi（320k）拿直链下载。
...
async function downloadQq(
  ...
  const purl = parseQqPurl(new TextDecoder().decode(vkeyRes.data));
  if (!purl) {
    return { ok: false, message: song.vip
      ? "该 QQ 歌曲为 VIP 曲目：需要绿钻账号的有效登录 Cookie（免费歌曲无需 Cookie；Cookie 从浏览器「网络」面板的 u.y.qq.com 请求头复制）"
      : "获取 QQ 播放地址失败（可能接口变更或区域限制）" };
  }
  const audioUrl = /^https?:\/\//i.test(purl) ? purl : `https://dl.stream.qqmusic.qq.com/${purl}`;
  ...
  const audio = audioRes?.data;
  if (!audio || audio.byteLength < MIN_AUDIO_BYTES) return { ok: false, message: "下载失败（可能付费受限：VIP 歌曲需绿钻账号的有效 Cookie）" };
...
/** 下载酷狗歌曲到音频文件夹（免登录，免费通道）：getSongInfo.php → trackercdn v2 兜底 */
...
/** 下载酷我歌曲到音频文件夹（免登录，mobi.s 车载通道）：按 128→320→flac 逐级降级取首个可用直链 */
...
async function httpPost(
  ...
  try {
    const r = (window as any).require;
    if (typeof r === "function" && (r("https")?.request || r("http")?.request)) {
      return await nodePost(url, body, extraHeaders, signal, timeoutMs);
    }
  } catch { /* 回退 fetch */ }
  return fetchPost(url, body, extraHeaders, signal, timeoutMs);
}
...
/** 下载目标文件夹：并入「音频文件夹」（歌单目录），下载即落歌单；为空则写 vault 根。
 *  库外盘符绝对路径的音频文件夹 → 返回盘符绝对目标路径（写盘走 fs 直写）；否则 vault 内相对路径。 */
async function resolveDownloadTarget(plugin: MusicManager, filename: string): Promise<string> {
...
async function fileExists(plugin: MusicManager, targetPath: string): Promise<boolean> {
...
/** 解析下载目标：同名文件已存在时追加来源后缀（`artist - name (QQ).mp3`），仍冲突则递增 `(2) (3)…` 直到可用 */
async function resolveDownloadTargetUnique(plugin: MusicManager, filename: string, source: string): Promise<string> {
...
export async function writeAudioToVault(
  ...
  try {
    if (existing instanceof TFile) {
      await app.vault.modifyBinary(existing, asArrayBuffer(data));
    } else {
      await app.vault.createBinary(targetPath, asArrayBuffer(data));
    }
...
async function rollback(app: App, existing: TAbstractFile | null, backup: ArrayBuffer | null): Promise<void> {
  try {
    if (existing instanceof TFile && backup) {
      await app.vault.modifyBinary(existing, asArrayBuffer(new Uint8Array(backup)));
    }
