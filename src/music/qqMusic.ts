...
export interface QqSong {
  ...
  coverUrl?: string;
  /** 音频格式（结果行格式胶囊）：按搜索档位推断 mp3/m4a/flac；无档位信息为 undefined */
  ext?: "mp3" | "m4a" | "flac";
}
...
/** 剥 JSONP 包裹（`callback({...})` / 前置注释），返回纯 JSON 文本 */
...
/** 搜索结果项 → QqSong（行内显示实际下载档位 size128/128k，不虚标最高档） */
function mapQqSong(s: any): QqSong | null {
  ...
  // 格式胶囊推断：有 mp3 档（128/320）→ mp3；否则有无损 flac → flac；否则有 aac/m4a → m4a
  ...
  return {
    ...
    // 下载付费字段 pay.dowload（1=仅 VIP、100=付费单曲/试听）
...
/** 解析 client_search_cp 响应（可能是 JSONP 包裹），返回歌曲列表 */
export function parseQqSearchResponse(raw: string): QqSong[] {
  ...
  return list.map((s: any) => mapQqSong(s)).filter((s: QqSong | null): s is QqSong => s !== null);
}
...
// --- 推荐歌单：fcg_get_diss_by_tag 歌单广场（免登录）+ 歌单歌曲 ---
...
export function parseQqLyricResponse(raw: string): string | null {
  try {
    const data = JSON.parse(stripJsonp(raw));
...
export function parseQqDissDetail(raw: string): QqSong[] {
  ...
  if (!Array.isArray(list)) return [];
  return list.map((s: any) => mapQqSong(s)).filter((x): x is QqSong => x !== null);
}
...
/** 构造批量歌单歌曲数请求体（musicu.fcg 多 req）：每个歌单一个 req（`song_num=1`+`onlysonglist=1` 最小响应） */
...
/** 解析批量歌单歌曲数：`req_{i+1}.data.total_song_num` 与入参 ids 一一对应 */
...
/** 解析 uniform_get_Dissinfo 响应（`req_1.data.songlist[]`）：mid=songmid/name/singer[].name/album.mid/file.size_128mp3/pay.down/interval */
export function parseQqDissDetailV2(raw: string): QqSong[] {
  ...
  for (const it of list) {
    ...
    // 音质档字段名为 size_128mp3 / size_320mp3 / size_flac；行内显示实际下载档位 128k
    ...
    out.push({
      ...
      // uniform_get_Dissinfo 的下载付费字段为 pay.down（1=仅 VIP、100=付费单曲/试听）
