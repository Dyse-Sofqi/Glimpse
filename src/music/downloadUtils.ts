/**
 * 下载相关纯逻辑。
 * 文件名校验与构造、多源搜索结果相似度排序、网易云 VIP 过滤、时长格式化。
 */
...
/**
 * 网易云歌曲是否可下载（VIP 过滤）。
 * fee 语义（社区共识）：0=免费；1=VIP；4=数字专辑购买；8=非会员可免费播放（标准音质）。
 * fee 缺省视为可下；仅 1/4 等真正受限的记为 VIP——8 是免费可下（标准档），
 * 误标会造成「标了 VIP 却能下载/试听」的困惑。
 */
export function isNeteaseDownloadable(fee: number | undefined): boolean {
  if (fee === undefined || fee === 0 || fee === 8) return true;
  return !(fee > 0);
}
