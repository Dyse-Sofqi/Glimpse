/**
 * MP4/M4A 原子（atom）解析与标签写入（纯 TS，无依赖）。
 * 读取：©nam/©ART/©alb/©lyr 文本原子 + covr 封面 + mvhd 时长。
 * 写入：替换 moov.udta.meta.ilst 中的目标原子，moov 位于 mdat 之前时同步修正 stco/co64 chunk 偏移。
 */
...
/** 从 M4A(MP4) 字节流提取指定文本 atom 的 data 文本（如 ©nam/©ART/©alb/©lyr）。非 M4A 或无该原子返回 null。 */
...
/** 遍历整文件修正 stco/co64 chunk 偏移（写入使 mdat 位移后必须调用） */
