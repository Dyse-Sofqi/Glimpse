/**
 * OGG 容器的 Vorbis/Opus comment 标签解析（jsmediatags 不支持 OGG）。
 */
import { decodeUtf8, parseVorbisComments } from "./vorbis";
...
/** 从 OGG 字节流解析 Vorbis/Opus comment 标签。非 OGG/无 comment 返回 null。 */
export function extractOggTags(bytes: Uint8Array | ArrayBuffer): OggTags | null {
  try {
    ...
    const map = parseVorbisComments(data, idx);
