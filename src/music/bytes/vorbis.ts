/**
 * FLAC / OGG 的 Vorbis comment 解析。
 * 提取 LYRICS 与常规标签字段（TITLE/ARTIST/ALBUM/DATE/GENRE 等）。
 */
...
/**
 * 解析 Vorbis comment 结构：vendor 长度(LE u32)+vendor+评论数(LE u32)+评论列表("KEY=value")。
 * 返回大写 KEY → value 映射；解析失败返回空映射。
 */
export function parseVorbisComments(data: Uint8Array, start: number): Map<string, string> {
...
/**
 * 从 FLAC 字节流解析 Vorbis comment 标签。
 * FLAC 布局：'fLaC' 标志 → 元数据块（1 字节头：last 标志+类型，3 字节长度）。
 * 类型 4 = VORBIS_COMMENT。非 FLAC 返回 null。
 */
export function extractFlacTags(bytes: Uint8Array | ArrayBuffer): Map<string, string> | null {
  try {
    ...
    while (offset + 4 <= b.length) {
      ...
      if (type === 4) { // VORBIS_COMMENT
        return parseVorbisComments(b, offset);
      }
      offset += len;
      if (isLast) break;
    }
...
/** 从 FLAC 字节流解析 Vorbis comment 的 LYRICS 字段。无 LYRICS 或非 FLAC 返回 null。 */
export function extractFlacLyrics(bytes: Uint8Array | ArrayBuffer): string | null {
  const tags = extractFlacTags(bytes);
  const lyrics = tags?.get("LYRICS");
