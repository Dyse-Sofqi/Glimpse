...
- **宽度自适应**：按内容最宽行自动适配宽度（切换上一项/下一项不增宽），宽度钳制视口上限，长文本换行不溢出；右缘可拖拽调整并自动锁定
...
- **透明度**：设置中可调字体透明度（默认 25%）与背景透明度（默认 50%）；背景色与边框常显，透明度实时生效
- **渲染样式**：内容复用主题与自定义 CSS（标题、代码块、内联格式等），仅字体大小由提词器控制
