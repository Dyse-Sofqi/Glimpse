...
- **穿透锁定**：窗口整体穿透鼠标（仅保留交互按钮），不遮挡编辑；背景全透明统一由「隐藏背景」按钮控制，穿透锁定本身不再改动背景；锁定状态下按钮栏同样仅在鼠标悬停时显示，移出窗口自动隐藏，「跟踪光标」按钮随其他非交互按钮一并隐藏
...
- **双击定位**：双击文本显示域，编辑器光标跳到捕获文本所在行（高亮模式跳到当前匹配项所在行）并聚焦，目标行滚动至视口中央
