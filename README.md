...
- **文档绑定**：将窗口固定到某个文档，不再跟随活动文档
- **穿透锁定**：窗口整体穿透鼠标（仅保留交互按钮），不遮挡编辑
- **宽度自适应**：按内容最宽行自动适配宽度（切换上一项/下一项不增宽），宽度钳制视口上限，长文本换行不溢出；右缘可拖拽调整并自动锁定
- **拖拽吸附**：贴近视口边缘或中心线时自动吸附，附辅助线提示
- **字体大小**：32/40/50/64/80px 五档循环
- **工具栏**：模式切换为文本按钮（显示「逐行提取」/「高亮提取」）；上一项/下一项（`arrow-big-left`/`arrow-big-right`）、宽度锁定（`move-horizontal`）、穿透锁定（`lock`/`unlock`）、字体档位图标（`heading-1`~`heading-5`）均为语义化 lucide 图标；设置按钮直达提词器设置页
- **透明度**：设置中可调字体透明度（默认 25%）与背景透明度（默认 50%）
